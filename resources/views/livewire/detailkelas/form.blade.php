    ...
    <div class="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
      ...
      <div class="inline-block align-bottom bg-white rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full" role="dialog" aria-modal="true" aria-labelledby="modal-headline">
        ...
        <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
            <div>
              <h1 class="font-bold text-center mb-4">CREATE DETAIL KELAS</h1>
            </div>
              <div>
                    <div class="mb-2">
                      <input wire:model="dkelas_id" type="hidden" class="shadow appearance-none border rounded w-full py-2 px-3 text-blue-900" >
                    </div>
                    <div class="mb-2">
                      <label for="kelas_id" class="block">Kelas</label>
                      {{-- <select wire:model="kelas_id" name="kelas_id" id="kelas_id" class="shadow appearance-none border rounded w-full py-2 px-3 text-blue-900">
                          @foreach ($kelas as $kls)
                              <option value=" {{$kls->id}} "> {{$kls->name}} </option>
                          @endforeach
                      </select> --}}
                      {{ Form::select('kelas_id',$kelas,null,
                        ['class' => 'shadow appearance-none border rounded w-full py-2 px-3 text-blue-900','id' => 'kelas_id','wire:model'=>'kelas_id','placeholder'=>'Pilih Kelas'])}}
                        @error('kelas_id') <h1 class="text-red-500">{{$message}}</h1>@enderror

                    </div>
                    <div class="mb-2">
                        <label for="periode_id" class="block">Periode</label>
                        {{ Form::select('periode_id',$periode,null,
                          ['class' => 'shadow appearance-none border rounded w-full py-2 px-3 text-blue-900','id' => 'periode_id','wire:model'=>'periode_id','placeholder'=>'Pilih Periode'])}}
                          @error('periode_id') <h1 class="text-red-500">{{$message}}</h1>@enderror

                      </div>
                      <div class="mb-2">
                        <label for="walas_id" class="block">Wali Kelas</label>
                        {{ Form::select('walas_id',$walas,null,
                          ['class' => 'shadow appearance-none border rounded w-full py-2 px-3 text-blue-900','id' => 'walas_id','wire:model'=>'walas_id','placeholder'=>'Pilih Walas'])}}
