    ...
    <div class="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
      ...
      <div class="inline-block align-bottom bg-white -lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full" role="dialog" aria-modal="true" aria-labelledby="modal-headline">
        ...
        <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
            <div>
              <h1 class="font-bold text-center mb-4">INPUT ASRAMA</h1>
            </div>
              <div>
                  <div class="mb-2">
                      <input wire:model="asramaId" type="hidden" class="shadow appearance-none border w-full py-2 px-3 text-blue-900" placeholder="Input Asrama">
                  </div>
                  <div class="mb-2">
                    <label for="genre" class="block py-1">Jenis Asrama</label>
                    {{ Form::select('genre',['1' => 'Asrama Laki-Laki','2'=>'Asrama Perempuan'],null,
                      ['class' => 'shadow appearance-none border w-full py-2 px-3 text-blue-900 text-sm','id' => 'genre','wire:model'=>'genre','placeholder'=>'- Pilih jenis asrama -'])}}
                      @error('genre') <h1 class="text-red-500">{{$message}}</h1>@enderror
                  </div>
                  <div class="mb-2">
                      <label for="name" class="block py-1">Nama Asrama</label>
                      <input wire:model="name" name="name" class="shadow appearance-none border w-full py-2 px-3 text-blue-900 text-sm" placeholder="Input asrama">
